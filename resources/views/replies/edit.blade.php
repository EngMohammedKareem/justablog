<x-app-layout>
    <div class="min-h-screen bg-gradient-home flex items-center justify-center">
        <div class="w-full max-w-lg p-8 bg-gray-800 bg-opacity-90 rounded-lg shadow-lg">
            <h1 class="text-3xl font-bold text-white mb-6">Edit Reply</h1>
            ...
            <!-- Edit Comment Form -->
            <form method="POST" action="{{ route('posts.comments.replies.update',[$post, $comment, $reply]) }}">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <textarea id="body" name="body" rows="4" class="block mt-1 w-full p-2 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-400" required>{{ $reply->body }}</textarea>
                </div>
